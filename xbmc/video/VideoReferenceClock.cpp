...
void CVideoReferenceClock::Process()
{
  ...
  while(!m_bStop)
  {
    ...
    if (SetupSuccess)
    {
      ...
      //m_CurrTime = 0;              //init to 0 to make it clear the time is not related to system time now
      SingleLock.Leave();
...
void CVideoReferenceClock::UpdateClock(int NrVBlanks, int64_t measuredVblankTime)
{
  if (NrVBlanks > 0) //update the clock with the adjusted frequency if we have any vblanks
  {
    ...
    CSingleLock SingleLock(m_CritSection);
    double increment = (double)NrVBlanks * m_ClockTickInterval;
  ...
  // and maintain m_VblankSampleConfidence and m_SampledVblankCount to track our progress
{
       ...
       if ( lm_VblankSampleConfidence > maxConfidence ) lm_VblankSampleConfidence = maxConfidence;
       else if ( lm_VblankSampleConfidence < minConfidence ) lm_VblankSampleConfidence = minConfidence;
       ...
       else // we have at least 3 samples
       {
          ...
          if ( lm_VblankSampleConfidence >= VBLANKTIMESVERYCONFIDENT ) max_lateness = lm_PVblankInterval * 80 / 100;
          else if ( lm_VblankSampleConfidence >= VBLANKTIMESQUITECONFIDENT ) max_lateness = lm_PVblankInterval * 65 / 100;
       ...
       else if ( m_SampledVblankCount > min(20, VBLANKTIMESSIZE) ) //at least 20 or VBLANKTIMESSIZE samples
       {
          ...
          if ( i_t != i_ts )  // safety sanity check - should never fail
          {
              ...
              if ( total_pvblanks ) // division by zero safety check - should never fail
              {
                  ...
                  int integral;
                  //if ( m_SampledVblankCount > 5000 ) integral = 1000;
                  if ( m_SampledVblankCount > 1000 ) integral = 500;
...
int64_t CVideoReferenceClock::GetTime(int64_t* InterpolatedTime /*= NULL */)
{
  CSingleLock SingleLock(m_CritSection);
  ...
  if (m_UseVblank)
  {
    ...
    double clockTickInterval = m_ClockTickInterval;
    ...
    if (InterpolatedTime)
    {
        ...
        *InterpolatedTime = CurrTime + (int64_t)increment;
    }

    int64_t vrnew = CurrTime + (int64_t)((double)vBlanksMissed * clockTickInterval);

#if _DEBUG
  static int64_t vrold;
  if(vrold > vrnew)
    CLog::Log(LOGWARNING, "CVideoReferenceClock::GetTime CLOCK GONE BACK pVBlankInterval: %"PRId64" vBlankSampleConfidence: %i missed_tol: %"PRId64" refreshRate: %"PRId64" Now: %"PRId64" vBlankTime: %"PRId64" clockTickInterval: %f vBlanksMissed: %i vrnew: %"PRId64" vrold: %"PRId64"", pVBlankInterval, vBlankSampleConfidence, missed_tol, refreshRate, Now, vBlankTime, clockTickInterval, vBlanksMissed, vrnew, vrold);
  vrold = vrnew;
#endif
...
int CVideoReferenceClock::GetRefreshRate(double* interval /*= NULL*/)
{
  ...
  if (m_UseVblank)
  {
    double clock_interval = m_ClockSpeed / m_RefreshRate * m_SystemFrequency;
    if ( clock_interval != m_ClockTickInterval ) m_ClockTickInterval = clock_interval;
    if (interval) // interval is returned in seconds
...
//if WaitedSysTime supplied return the system time duration we actually waited or 0 if we did not wait
int64_t CVideoReferenceClock::Wait(int64_t Target /* = 0 */, int64_t* InterpolatedTime /*= NULL */, int64_t* WaitedSysTime /* = NULL */)
{
  int           SleepTime = 0;
  int64_t NowStart;
  int64_t Now;
  ...
  if (m_UseVblank) //when true the vblank is used as clock source
  {
    ...
    if (CurrTime >= Target)
    {
       // change to just return tick time and do nothing clever
       if (InterpolatedTime)
          *InterpolatedTime = CurrInterpolatedTime;
       return CurrTime;
    }
    ...
    while (waitMs > safeDistance )
    {
      ...
      if (SleepTime == 50)  // if we have slept the full 50ms then check time again - just in case unexpected events occur
      {
         ...
         {
            if (WaitedSysTime)
                *WaitedSysTime = CurrentHostCounter() - NowStart;
            if (InterpolatedTime)
               *InterpolatedTime = CurrTime;
            return CurrTime;
         }
    ...
    CurrTime = GetTime();  // hopefully we have waited long enough - if not just keep checking every 2 ms for around one vblank time beyond target system time then give up (timeout) as something is wrong
     ...
     {
        // if more than 2 ms still to go to estimated vblank time then simply sleep that amount, else sleep 2
        if (remainMs > 1)
        {
          m_VblankEvent.Reset();
    ...
    // return in WaitedSysTime if supplied our total wait 
    if (WaitedSysTime)
       *WaitedSysTime = CurrentHostCounter() - NowStart;
    if (InterpolatedTime)
       *InterpolatedTime = CurrTime;
  ...
  else
  {
    ...
    Now = CurrentHostCounter();
    if (SleepTime > 0 && WaitedSysTime)
       *WaitedSysTime = Now - NowStart;
