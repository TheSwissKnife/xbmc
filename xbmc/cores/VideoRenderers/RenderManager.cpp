...
//  m_pClock = clock;
  ...
  YV12Image image;
  source = m_pRenderer->FlipFreeBuffer();
  if (source < 0)
  {
...
void CXBMCRenderManager::UpdateDisplayInfo()
{
  ...
  bool bFramePtsChange = false;
  if (m_renderinfo.framepts != m_displayinfo[0].framepts);
     bFramePtsChange = true;
  ...
  //CLog::Log(LOGDEBUG, "CXBMCRenderManager::UpdateDisplayInfo 3 m_preflipclock: %f m_postflipclock: %f m_postrenderclock: %f m_renderinfo.framepts: %f m_displayinfo[0].framepts: %f m_renderinfo.frameplayspeed: %i", m_preflipclock, m_postflipclock, m_postrenderclock, m_renderinfo.framepts, m_displayinfo[0].framepts, m_renderinfo.frameplayspeed);
  ...
  //estimate if we got long/short display frames
...
bool CXBMCRenderManager::Drain()
{
  ...
  int index = m_pRenderer->GetNextRenderBufferIndex();
  if (index == -1)
    return true;
