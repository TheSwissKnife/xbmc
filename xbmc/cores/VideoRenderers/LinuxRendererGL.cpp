...
bool CLinuxRendererGL::Configure(unsigned int width, unsigned int height, unsigned int d_width, unsigned int d_height, float fps, unsigned flags)
{
  ...
  if ((prev_resolution == m_resolution) && (prev_sourceFrameRatio == m_sourceFrameRatio) && (prev_sourceHeight == m_sourceHeight) && (prev_sourceWidth == m_sourceWidth) && (prev_iFlags | CONF_FLAGS_FULLSCREEN == m_iFlags | CONF_FLAGS_FULLSCREEN )) 
  {
     CLog::Log(LOGDEBUG, "CLinuxRenderer::Configure fps only change with no resolution change, no need to unconfigure");
     return true;
  }
